...
def CheckLineAlignment(image,frame):
    ...
    try:
        ...
        for rho,theta in lines[0]:
            ...
            if np.abs(dy) > 20:
                ...
                if np.sign(dy) == 1:
                    print "Line is tilted to the left\n"
                    return 'l'
                else:
                    print "Line is tilted to the left\n"
                    return 'r'
            else:
                print "Line is aligned\n"
                return 's'
    except:
        return 's'
...
def drawCenterOfMass(image,frame):
    ...
    try:
        ...
        if len(cnts) > 0:
            ...
            if(all(value == 0 for value in M.values())):
                ...
                if radius > 10:
                    ...
                    # First calculate the focal length using known width of object (W), distance from camera (D)
                    # Then apparent width in pixels (P = diameter). F = (P*D)/W
                    # focal_length = diameter*300/65
                    # print "focal_length = ", focal_length

                    focal_length = 650
                    # After calculating the focal_length, calculate the approximate distance to the object using
                    # D' = (W*F)/P
                    calculated_cam_dist = 65*focal_length/diameter
                    print "calculated_distance = ", cam_dist

                    theta = 0.3
                    calculated_floor_dist = calculated_cam_dist*cos(theta)
        ...
        cv2.putText(frame, "dx: {}, dy: {}".format(dX, dY),(10, image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX,0.35, (0, 0, 255), 1)
...
def main():
    ...
    motion.angleInterpolation("HeadPitch", 0.3, 3.0, True)
