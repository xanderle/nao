...
class FallDetectionModule(ALModule):
    ...
    def FallDetected(self, arg1, arg2):
        """ Mandatory docstring.
          comment needed to create a new python module
        """
        ...
        tts.say("Nao! Down! Nao! Down!")

        # Send robot to Stand Init
        postureProxy.goToPosture("StandInit", 0.5)
        tts.say("I'm back in the game!")
...
class FallingDetectionModule(ALModule):
    """ Mandatory docstring.
      comment needed to create a new python module
    """
    def __init__(self, name):
        ALModule.__init__(self, name)
        print 'initialise falling event'
        memory.subscribeToEvent("robotIsFalling",
            "FallingDetection",
            "FallingDetected")
    def FallingDetected(self, *_args):
        """ Mandatory docstring.
          comment needed to create a new python module
        """
        memory.unsubscribeToEvent("robotIsFalling",
            "FallingDetection")
        # Send robot to Stand Init
...
def feed(motionBool):
    ...
    # global FallingDetection
    FallDetection = FallDetectionModule("FallDetection")
    # FallingDetection = FallingDetectionModule("FallingDetection")
    while True:
